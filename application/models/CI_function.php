<?php
	class CI_function extends CI_Model{
			...
			if($menu != NULL){
				...
				foreach ($menu as $key => $val) {
					...
					$data .= '<tr>
                        ...
                        <td><a href="'.ADMIN_MENU_URL.'edit/'.$val['id'].'">';
                        if($parentid > 0){ $data .= $option; }
                        $data .= $val['name'].'</a></td>
                        <td class="text-center">'.date('d/m/Y',strtotime($val['created_at'])).'</td>
                        <td width="100" class="text-center"><input onkeyup="changeSort(this,'.$val['id'].');" type="text" class="form-control text-center" value="'.$val['sort'].'" data-control="menu"></td>
                        <td class="text-center">
                            <div class="checkbox checkbox-success">
                                <input onchange="checkShow('.$val['id'].');"'; if($val['publish'] == 1){  $data .= 'checked '; } $data .= 'type="checkbox" name="publish" class="publish'.$val['id'].'" value="1" data-control="menu">
                                <label for="publish'.$val['id'].'"></label>
                            </div>
                        </td>
                        <td class="text-center">
                            <div class="checkbox checkbox-success">
                                <input onchange="checkShowMain('.$val['id'].');"'; if($val['main'] == 1){  $data .= 'checked '; } $data .= 'type="checkbox" name="main" class="main'.$val['id'].'" value="1" data-control="menu">
                                <label for="main'.$val['id'].'"></label>
                            </div>
                        </td>
                        <td class="text-center">
                            <div class="checkbox checkbox-success">
                                <input onchange="checkShowLeft('.$val['id'].');"'; if($val['left'] == 1){  $data .= 'checked '; } $data .= 'type="checkbox" name="left" class="left'.$val['id'].'" value="1" data-control="menu">
                                <label for="left'.$val['id'].'"></label>
                            </div>
                        </td>
                        <td class="text-center">
                            <div class="checkbox checkbox-success">
                                <input onchange="checkShowHome('.$val['id'].');"'; if($val['home'] == 1){  $data .= 'checked '; } $data .= 'type="checkbox" name="home" class="home'.$val['id'].'" value="1" data-control="menu">
                                <label for="home'.$val['id'].'"></label>
                            </div>
                        </td>
                        <td class="text-center">
                            <div class="checkbox checkbox-success">
                                <input onchange="checkShowHot('.$val['id'].');"'; if($val['hot'] == 1){  $data .= 'checked '; } $data .= 'type="checkbox" name="hot" class="hot'.$val['id'].'" value="1" data-control="menu">
                                <label for="hot'.$val['id'].'"></label>
                            </div>
                        </td>
                        <td class="text-center">
                            <div class="checkbox checkbox-success">
                                <input onchange="checkShowSearch('.$val['id'].');"'; if($val['search'] == 1){  $data .= 'checked '; } $data .= 'type="checkbox" name="search" class="search'.$val['id'].'" value="1" data-control="menu">
                                <label for="search'.$val['id'].'"></label>
                            </div>
                        </td>
                        <td class="text-center">
                            <div class="checkbox checkbox-success">
                                <input onchange="checkShowFooter('.$val['id'].');"'; if($val['footer'] == 1){  $data .= 'checked '; } $data .= 'type="checkbox" name="footer" class="footer'.$val['id'].'" value="1" data-control="menu">
                                <label for="footer'.$val['id'].'"></label>
                            </div>
                        </td>
                        <td class="text-center">
		...
		public function getCateBreadcrumb($parentid){
			$cate = $this->query_sql->select_row('tbl_menu', 'id, name, alias, parentid', array('id' => $parentid));
			if($cate != NULL){
				$result['cate_name1'] = $cate['name'];
				$result['cate_alias1'] = $cate['alias'];
				$cate2 = $this->query_sql->select_row('tbl_menu', 'id, name, alias, parentid', array('id' => $cate['parentid']));
				if($cate2 != NULL){
					$result['cate_name2'] = $cate2['name'];
					$result['cate_alias2'] = $cate2['alias'];
					$cate3 = $this->query_sql->select_row('tbl_menu', 'id, name, alias, parentid', array('id' => $cate2['parentid']));
					if($cate3 != NULL){
		...
		public function getArrayProductID($cateID){
			$result = array();
			$arrayCateID = $this->CI_function->getListMenuId($cateID);
			//get array productID
			$arrayProductIDByCateID = $this->query_sql->select_array_wherein('tbl_product_menu','productID',NULL,'cateID',$arrayCateID);
			if($arrayProductIDByCateID != NULL){
				foreach ($arrayProductIDByCateID as $key => $val) {
					array_push($result,$val['productID']);
				}
			}
	    ...
	    function getListMenuId($parentid){
			$result[] = $parentid;
			$menu_child = $this->query_sql->select_array('tbl_menu','id',array('publish' => 1, 'parentid' => $parentid));
			if($menu_child != NULL){
				foreach ($menu_child as $key => $val) {
					$result[] = (int)$val['id'];
					$menu_child_2 = $this->query_sql->select_array('tbl_menu','id',array('publish' => 1, 'parentid' => $val['id']));
